"""
Faux serveur — utilisé pour tester la GUI client sans le vrai serveur.

Usage :
    python -m tests.manual.fake_server <room_id> [--scenario lobby|write|draw|guess|reveal|full]

Comportement selon le scénario :
  - lobby  : reste en LOBBY, attend
  - write  : passe en WRITE après 3s
  - draw   : enchaîne LOBBY -> WRITE -> DRAW (round 1)
  - guess  : enchaîne jusqu'à GUESS (round 2)
  - reveal : enchaîne jusqu'à REVEAL avec albums fakes
  - full   : enchaîne tout, à intervalles courts (pour test rapide)

Le faux serveur publie aussi des albums préfabriqués pour que les écrans
draw / guess / reveal aient quelque chose à afficher.
"""
...
# Joueurs imaginaires pour les tests UI
...
class FakeServer:
    def __init__(self, room_id: str):
        ...
        self.mqtt = GarticMqttClient(
            ...
            will_topic=topics.t_server_presence(room_id),
            will_payload={"status": PresenceStatus.OFFLINE.value, "pseudo": "__server__", "ts": 0},
            will_qos=1,
            will_retain=True,
        )
    ...
    def on_ready(self):
        ...
        self.mqtt.publish_json(
            topics.t_server_presence(self.room_id),
            {"status": PresenceStatus.ONLINE.value, "pseudo": "__server__", "ts": int(time.time())},
            qos=1, retain=True,
        )
        # Publier aussi des présences fake pour que la GUI ait du contenu
        for p in FAKE_PLAYERS:
            self.mqtt.publish_json(
                topics.t_player_presence(self.room_id, p),
                {"status": PresenceStatus.ONLINE.value, "pseudo": p, "ts": int(time.time())},
                qos=1, retain=True,
            )
    ...
    def publish_fake_album(self, album_idx: int, round_n: int):
        """Publie un album fake pour que la GUI aie quelque chose à afficher."""
            ...
            payload = {
                ...
                "strokes": [
                    {"color": "#000000", "width": 3,
                     "points": [[100, 100], [200, 150], [300, 200], [350, 250]]},
                    {"color": "#ff0000", "width": 5,
                     "points": [[400, 300], [450, 320], [500, 350]]},
                ],
                "canvas_size": [800, 600],
    ...
    def scenario_draw(self, delay=3):
        ...
        time.sleep(delay)
        # Publier les albums de round 0 pour que les joueurs aient une phrase à dessiner
...
def main():
    ...
    parser.add_argument("--scenario", default="lobby",
                        choices=["lobby", "write", "draw", "guess", "reveal", "full"])
